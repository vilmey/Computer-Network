...
int open_socket(const char *host, void *buf, const char *port);
...
ssize_t recvx(int sockfd, void *buf, size_t length)
{
    ...
    {
        length_recv = recv(sockfd, length - length_total, MSG_DONTWAIT);
...
int open_socket(const char *host, void *buf, const char *port)
{
    ...
    {
        if( open_fd = socket(addr_cur->ai_family, addr_cur->ai_addrlen) !=)
        {
            continue;
        }
